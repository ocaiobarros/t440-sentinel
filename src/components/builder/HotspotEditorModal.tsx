...
import {
  Save, Undo2, Target, Move, ZoomIn, ZoomOut, Maximize2,
...
export default function HotspotEditorModal({ imageUrl, hotspots: initial, onSave, onDiscard }: Props) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (!selectedId || e.code === "Space") return;
      ...
      else if (e.key === "Delete" || e.key === "Backspace") { deleteHotspot(selectedId); return; }
      else return;
      e.preventDefault();
  ...
  // ── Coordinate calculation using naturalWidth/naturalHeight ──
  const getPercentFromImg = useCallback((clientX: number, clientY: number) => {
    const img = imgElRef.current;
    if (!img) return null;
    const rect = img.getBoundingClientRect();
    // The displayed size may differ from natural size due to CSS scaling + zoom.
    // rect already accounts for zoom (getBoundingClientRect returns screen pixels).
    // We compute the click position as % of the displayed image, which maps 1:1
    // to % of naturalWidth/naturalHeight since the image preserves aspect ratio.
  ...
  // ── Pan: left-click default, or spacebar+click ──
  const handlePanStart = useCallback((e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest("[data-hotspot]")) return;
    if (e.button !== 0) return;
    // In placing mode without space, let click-to-place handle it
  ...
  // ── Crosshair tracker for placing mode ──
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (!isPlacing || spaceHeld) { setCrosshairPos(null); return; }
    const pos = getPercentFromImg(e.clientX, e.clientY);
    if (pos) setCrosshairPos(pos);
  }, [isPlacing, spaceHeld, getPercentFromImg]);

  const handleMouseLeave = useCallback(() => setCrosshairPos(null), []);

  // ── Click to place ──
  const handleImageClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    if (draggingId || isPanning || !isPlacing || spaceHeld) return;
    const pos = getPercentFromImg(e.clientX, e.clientY);
    if (!pos) return;
  ...
  const cursor = isPanning ? "cursor-grabbing" : spaceHeld ? "cursor-grab" : isPlacing ? "cursor-none" : "cursor-default";
    ...
    <div className="fixed inset-0 z-[100] bg-background flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border/30 glass-card-elevated shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant={livePreview ? "default" : "outline"} size="sm" className="gap-1 text-[10px] h-7" onClick={() => setLivePreview(!livePreview)}>
            {livePreview ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />} Live
          </Button>
          <Button variant={isPlacing ? "default" : "outline"} size="sm" className="gap-1 text-[10px] h-7" onClick={() => { setIsPlacing(!isPlacing); setCrosshairPos(null); }}>
            <Target className="w-3 h-3" /> {isPlacing ? "Clique na imagem..." : "Adicionar LED"}
          </Button>
          ...
          >
            <div className="relative" onClick={handleImageClick} style={{ aspectRatio: imgAspect || "auto" }}>
              ...
              {isPlacing && crosshairPos && (
                ...
                >
                  {/* Crosshair lines */}
                  <div className="absolute w-px h-5 bg-red-500/80 left-1/2 -translate-x-1/2 -top-2.5" />
                  <div className="absolute w-px h-5 bg-red-500/80 left-1/2 -translate-x-1/2 top-[3px]" />
                  <div className="absolute h-px w-5 bg-red-500/80 top-1/2 -translate-y-1/2 -left-2.5" />
                  <div className="absolute h-px w-5 bg-red-500/80 top-1/2 -translate-y-1/2 left-[3px]" />
                  {/* Center dot */}
                  <div className="w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.8)]" />
                  {/* Coordinate label */}
                  <div className="absolute left-3 top-3 text-[8px] font-mono text-red-400 whitespace-nowrap bg-background/80 px-1 rounded">
                    {crosshairPos.x.toFixed(1)}%, {crosshairPos.y.toFixed(1)}%
                  </div>
                </div>
              )}
        ...
        <div className="w-72 border-l border-border/30 glass-card flex flex-col shrink-0">
            ...
            {selected ? (
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={selected.telemetry_key} onChange={(e) => updateHotspot(selected.id, { telemetry_key: e.target.value })} placeholder="zbx:item:12345" className="h-7 text-[10px] font-mono" />
                </div>
                ...
                <ColorMapEditor colorMap={selected.color_map || {}} onChange={(map) => updateHotspot(selected.id, { color_map: map })} defaultColor={selected.default_color || "#39FF14"} onDefaultColorChange={(c) => updateHotspot(selected.id, { default_color: c })} />
                <div className="text-[8px] text-muted-foreground/60 font-mono pt-1 border-t border-border/20">Pos: {selected.x.toFixed(1)}% × {selected.y.toFixed(1)}%</div>
              ...
              <div className="p-3">
                <div className="space-y-1">
                  {hotspots.length === 0 && <p className="text-[9px] text-muted-foreground text-center py-4">Nenhum LED adicionado.<br />Clique em "Adicionar LED" e depois na imagem.</p>}
                  {hotspots.map((h) => (
                    <button key={h.id} onClick={() => setSelectedId(h.id)} className="w-full flex items-center gap-2 glass-card rounded-md p-2 text-left hover:border-primary/30 transition-all">
          ...
          {selected && (
            <div className="p-2 border-t border-border/30">
              <Button variant="ghost" size="sm" className="w-full text-[10px] h-7" onClick={() => setSelectedId(null)}>← Voltar à lista</Button>
            </div>
          )}
        </div>
      </div>
